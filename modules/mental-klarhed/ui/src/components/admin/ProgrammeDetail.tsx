...
export function ProgrammeDetail({ programmeId, onBack }: Props) {
    const api = useMentalKlarhedApi();
    const [programme, setProgramme] = useState<Programme | null>(null);
    const [loading, setLoading] = useState(true);
    const [actionLoading, setActionLoading] = useState<string | null>(null);
    ...
    const reload = () => {
        setLoading(true);
        api.getProgramme(programmeId).then(setProgramme).finally(() => setLoading(false));
    };
                        ...
                        <div className="flex gap-2 flex-wrap">
                            ...
                            {session.status === 'MATERIAL_GENERATED' && (
                                <button
                                    ...
                                    className="text-xs px-3 py-1.5 bg-primary text-primary-foreground rounded hover:opacity-90 disabled:opacity-50"
                                >
                                    {actionLoading === `material-${session.sessionNumber}` ? '...' : 'Send PDF + video-manus til klient'}
                                </button>
                            )}
                        </div>
                    </div>
