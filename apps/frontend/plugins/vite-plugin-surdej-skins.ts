...
interface ScannedSkinRegistry {
    skins: ScannedSkin[];
    errors: Array<{ skin: string; error: string }>;
    generatedAt: string;
}

export function surdejSkins(options: Options = {}): Plugin {
...
function scanSkins(skinsPath: string, verbose: boolean): ScannedSkinRegistry {
    const registry: ScannedSkinRegistry = {
        skins: [],
        errors: [],
...
function parseSkinManifest(filePath: string, dirName: string): ScannedSkin {
    ...
    const defaultMode = extractStr(content, 'defaultMode') as 'light' | 'dark' | undefined;

    const sidebar = extractSidebarItems(content);

    const branding: ScannedSkin['branding'] = { appName };
    if (logo) branding.logo = logo;
    if (primaryColor) branding.primaryColor = primaryColor;
    if (fontFamily) branding.fontFamily = fontFamily;

    const result: ScannedSkin = { id, name, branding, sidebar };
    if (description) result.description = description;
    if (defaultMode) result.theme = { defaultMode };

    return result;
}

function extractStr(content: string, field: string): string | undefined {
    const re = new RegExp(field + ":\\s*['\"]([^'\"]*)['\"]");
    const m = content.match(re);
    return m ? m[1] : undefined;
}

function extractSidebarItems(content: string): Array<{ commandId: string; group?: string }> {
    const sidebarMatch = content.match(/sidebar:\s*\[([\s\S]*?)]/);
    if (!sidebarMatch) return [];

    const items: Array<{ commandId: string; group?: string }> = [];
    const pattern = /\{\s*commandId:\s*['"]([^'"]+)['"]\s*(?:,\s*group:\s*['"]([^'"]+)['"])?\s*}/g;
    let m: RegExpExecArray | null;

    while ((m = pattern.exec(sidebarMatch[1])) !== null) {
        const item: { commandId: string; group?: string } = { commandId: m[1] };
        if (m[2]) item.group = m[2];
        items.push(item);
    }
...
function validateSkinManifest(manifest: ScannedSkin): string[] {
    const errors: string[] = [];
...
function generateModule(registry: ScannedSkinRegistry): string {
    const lines = [
