...
interface ScannedDomain {
    ...
    version?: string;
    commands?: string[];
    routes?: Array<{ path: string; label: string; commandId?: string }>;
    topologies?: Array<{ id: string; file: string; generated?: boolean }>;
...
export function surdejDomains(options: Options = {}): Plugin {
...
function scanDomains(domainsPath: string, verbose: boolean): ScannedRegistry {
...
function parseManifestFile(filePath: string, dirName: string): ScannedDomain {
...
function extractArr(content: string, field: string): string[] | undefined {
    const re = new RegExp(field + ':\\s*\\[([^\\]]*)\\]');
    const m = content.match(re);
...
function validateManifest(manifest: ScannedDomain): string[] {
    const errors: string[] = [];
    if (!manifest.id) {
...
function generateModule(registry: ScannedRegistry): string {
